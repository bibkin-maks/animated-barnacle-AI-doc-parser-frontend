...
const Chat = () => {
  ...
  const activeDocumentName =
    uploadedFile?.name || user?.document?.name || null;

  // Document status based on actual state
  const docStatus = uploadedFile
    ? { label: "Ready", color: "text-emerald-400" }
    : user?.document?.name
      ? { label: "Loaded", color: "text-cyan-400" }
      : { label: "None", color: "text-slate-500" };
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden font-montserrat text-white bg-transparent selection:bg-purple-500/30 flex flex-col">
      ...
      <header className="relative z-10 flex-none h-16 w-full flex items-center justify-between px-4 lg:px-6 border-b border-white/5 bg-[#0f1115]/50 backdrop-blur-xl">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <p className="text-sm font-bold text-white tracking-wide uppercase">DocuChat</p>
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <p className="text-[10px] text-slate-500 font-medium tracking-wider">Private Workspace</p>
            </div>
          </div>
        </div>

        <div className="flex items-center gap-2">
          {/* Context Panel Toggle — visible on desktop */}
          <button
            onClick={togglePanel}
            className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-all duration-200 text-xs font-medium"
            aria-label={isPanelOpen ? "Hide context panel" : "Show context panel"}
            title={isPanelOpen ? "Hide context panel" : "Show context panel"}
          >
            <svg
          ...
          {user && (
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                aria-label={showMobileContext ? "Close document panel" : "Open document panel"}
                title={showMobileContext ? "Close document panel" : "Open document panel"}
      ...
      <main className="relative z-10 flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        {/* LEFT: Upload Panel — collapsible on desktop */}
        <section
          className={`
            flex-none flex flex-col border-b lg:border-b-0 border-white/5 bg-[#0f1115]/80 backdrop-blur-xl lg:bg-[#0f1115]/30 lg:backdrop-blur-sm z-30
            absolute inset-0 lg:static w-full transition-all duration-300 ease-in-out
            ${showMobileContext ? "translate-y-0 flex" : "translate-y-full lg:translate-y-0 hidden lg:flex"}
            ${isPanelOpen ? "lg:w-[360px] lg:opacity-100 lg:border-r lg:border-white/5" : "lg:w-0 lg:opacity-0 lg:overflow-hidden lg:pointer-events-none"}
          `}
        >
          {/* Mobile Header */}
          <div className="lg:hidden flex justify-between items-center p-4 border-b border-white/5">
            <h2 className="font-bold text-white">Context &amp; Upload</h2>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar min-w-[360px]">
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-white/5">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Upload a PDF to establish context. The AI will answer your questions based on this document.
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-[#13161c]/80 border border-white/5 p-4 flex flex-col gap-3 shadow-lg">
              <div>
                <p className="text-[11px] uppercase tracking-widest text-slate-500 font-bold mb-3">Active Document</p>
                <div className="flex items-center gap-3 text-slate-200 bg-white/5 p-2 rounded-lg border border-white/5">
                  ...
                  <span className="font-medium truncate text-xs" title={activeDocumentName || "No file loaded"}>
                    {activeDocumentName || "No file loaded"}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-black/20 rounded-lg p-2 text-center border border-white/5">
                  ...
                  <p className={`text-[11px] font-semibold ${docStatus.color}`}>{docStatus.label}</p>
                </div>
                <div className="bg-black/20 rounded-lg p-2 text-center border border-white/5">
                  <p className="text-[11px] text-slate-500 uppercase">Privacy</p>
                  <p className="text-[11px] text-purple-400 font-semibold">Encrypted</p>
                </div>
              </div>
              ...
              {(uploadedFile) && (
                <button
                  ...
                  className="mt-1 w-full py-1.5 rounded-lg text-xs font-medium text-slate-400 hover:text-red-400 border border-white/5 hover:border-red-400/20 hover:bg-red-500/5 transition-all duration-200"
                >
                  Clear document
                </button>
              )}
        ...
        {/* RIGHT: Chat */}
        <section className="flex-1 flex flex-col min-h-0 bg-transparent relative z-10 min-w-0">
